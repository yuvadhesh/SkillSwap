...
const adminSettingsRoutes = require('./adminSettingsRoutes.cjs');
...
app.post('/api/profile/photo', upload.single('photo'), async (req, res) => {
  ...
  try {
    ...
    // Broadcast the updated photo to everyone
    io.emit('profile_update', { email: email.toLowerCase(), profilePhoto: photoUrl });
...
app.post('/api/profile/photo/remove', async (req, res) => {
  ...
  try {
    ...
    // Broadcast the removed photo to everyone
    io.emit('profile_update', { email: email.toLowerCase(), profilePhoto: '' });
...
io.on('connection', (socket) => {
  ...
  socket.on('send_message', async ({ sender, receiver, message, file }) => {
    ...
    try {
      ...
      if (message && message.startsWith('[VIDEO_CALL_LINK]:')) {
        const senderUser = await db.getUserByEmail(sender);
        const senderName = senderUser ? senderUser.name : sender;
        io.to(`global_${receiver.toLowerCase()}`).emit('new_notification', {
          type: 'video_invite',
          text: `Video call invite from ${senderName}`,
          roomName: message.substring('[VIDEO_CALL_LINK]:'.length),
