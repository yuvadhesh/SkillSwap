...
import { Camera, Mic, Monitor, AlertTriangle, ShieldCheck, Play, ArrowRight, Save, Clock, Trophy, Download, Lock, CameraOff, MicOff, ShieldAlert } from 'lucide-react';
...
export default function AssessmentRunner({ assessment, user, onExit }: AssessmentRunnerProps) {
  ...
  // Security Settings & Violation Thresholds
  const [securityLimits, setSecurityLimits] = useState({
    maxTabSwitches: 3,
  ...
  const securityLimitsRef = useRef(securityLimits);
  ...
  const aiCheckIntervalRef = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  // Fetch Security Limits from Admin Settings
  useEffect(() => {
    fetch(`${API_URL}/api/admin/settings`)
      ...
      .then(data => {
        if (data.success && data.settings) {
          ...
          setSecurityLimits(limits);
          securityLimitsRef.current = limits;
        }
      })
      .catch(() => {});
  }, []);
  ...
  const recordViolation = (type: string, category: 'tabSwitches' | 'fullscreenExits' | 'copyAttempts' | 'aiWarnings') => {
    ...
    const v = { 
      ...
      details: `Security log: ${type}`
    };

    violationsRef.current = [...violationsRef.current, v];
    setViolations((prev) => [...prev, v]);

    const limits = securityLimitsRef.current;
    const currentCount = (violationCountsRef.current[category] || 0) + 1;
    const updatedCounts = { ...violationCountsRef.current, [category]: currentCount };
    violationCountsRef.current = updatedCounts;
    setViolationCounts(updatedCounts);

    const maxLimit = category === 'tabSwitches' ? limits.maxTabSwitches
      : category === 'fullscreenExits' ? limits.maxFullscreenExits
      : category === 'copyAttempts' ? limits.maxCopyAttempts
      : limits.maxAiDetectionWarnings;

    if (currentCount >= maxLimit) {
      toast.error(`Security Limit Exceeded (${type}). Test being auto-submitted/terminated.`);
      if (limits.autoSubmit || limits.terminateAssessment) {
        setTimeout(() => {
          submitAssessment(limits.terminateAssessment);
        }, 500);
      }
    } else {
      toast.error(`Security Warning (${currentCount}/${maxLimit}): ${type}.`);
    }
  };

  // ---- Exam Mode Security Listeners ----
  ...
  const blockEvent = (e: Event) => {
    if (stageRef.current === 'running') e.preventDefault();
  };

  const onContextMenu = (e: MouseEvent) => {
    if (stageRef.current === 'running') {
      e.preventDefault();
      recordViolation('Right Click Attempt', 'copyAttempts');
    }
  };
  ...
  const onCopyCutPaste = (e: Event) => {
    if (stageRef.current === 'running') {
      e.preventDefault();
      const type = e.type === 'copy' ? 'Copy Attempt' : e.type === 'cut' ? 'Cut Attempt' : 'Paste Attempt';
      recordViolation(type, 'copyAttempts');
    }
  };
  ...
  const onKeyDown = (e: KeyboardEvent) => {
    ...
    // F12, Ctrl+Shift+I, Ctrl+Shift+J, Ctrl+Shift+C, Ctrl+U (Developer tools / View source)
    if (
      key === 'F12' ||
      (ctrl && shift && ['I', 'i', 'J', 'j', 'C', 'c'].includes(key)) ||
      (ctrl && ['U', 'u'].includes(key))
    ) {
      e.preventDefault();
      e.stopPropagation();
      recordViolation(`Developer Tools Shortcut Attempt (${key})`, 'copyAttempts');
      return;
    }

    // Ctrl+C, Ctrl+X, Ctrl+V, Ctrl+A, Ctrl+P, Ctrl+S
    if (ctrl && ['C', 'c', 'X', 'x', 'V', 'v', 'A', 'a', 'P', 'p', 'S', 's'].includes(key)) {
      e.preventDefault();
      e.stopPropagation();
      const label = key.toUpperCase() === 'C' ? 'Copy'
        : key.toUpperCase() === 'X' ? 'Cut'
        : key.toUpperCase() === 'V' ? 'Paste'
        : key.toUpperCase() === 'A' ? 'Select All'
        : key.toUpperCase() === 'P' ? 'Print'
        : 'Save Page';
      recordViolation(`Shortcut Attempt (${label})`, 'copyAttempts');
      return;
    }
  };
  ...
  const onVisibilityChange = () => {
    if (document.hidden && stageRef.current === 'running') {
      recordViolation('Tab Switched / Browser Minimized', 'tabSwitches');
    }
  };

  const onWindowBlur = () => {
    if (stageRef.current === 'running') {
      recordViolation('Window Blur / Focus Lost', 'tabSwitches');
    }
  };
  ...
  const onFullscreenChange = () => {
    if (!document.fullscreenElement && stageRef.current === 'running') {
      ...
      recordViolation('Fullscreen Exit', 'fullscreenExits');
  ...
  const onResize = () => {
    if (stageRef.current === 'running' && !document.fullscreenElement) {
      recordViolation('Screen Resize / Window Altered', 'fullscreenExits');
    }
  };

  // ---- AI Assistant Tool Extension Detection ----
  // NOTE: Only fires when a visible floating overlay is actively shown on screen.
  // Background extension DOM nodes (hidden/zero-size) are intentionally ignored to avoid false positives.
  const lastAiWarnTimeRef = useRef<number>(0);
  const reportedAiSelectorsRef = useRef<Set<string>>(new Set());

  const startAiDetection = () => {
    aiCheckIntervalRef.current = setInterval(() => {
      if (stageRef.current !== 'running') return;

      // Specific shadow-host or root-level containers injected by known AI overlays
      // These selectors are intentionally strict to avoid common false positives.
      const aiSelectors = [
        // Monica AI — injects a specific shadow root host
        'monica-root',
        // Sider — injects a specific ID
        'sider-sidebar-root',
        // Merlin
        'merlin-sidebar-root',
        // ChatGPT/OpenAI sidebar extensions typically use this
        '#chatgpt-sidebar-root',
        // MaxAI
        '#maxai-root',
        // Harpa
        '#harpa-root',
        // Generic floating AI assistant containers
        '[data-extension-overlay="true"]',
        '[data-ai-assistant-panel="open"]'
      ];

      const now = Date.now();
      // Cooldown: don't fire more than once every 30 seconds
      if (now - lastAiWarnTimeRef.current < 30000) return;

      for (const sel of aiSelectors) {
        try {
          // Check both regular elements and custom elements
          const el = sel.includes('#') || sel.includes('[') || sel.includes('.')
            ? document.querySelector(sel)
            : document.querySelector(sel) || document.getElementsByTagName(sel)[0];

          if (!el) continue;
          if (el.id === 'root' || el.id === 'app') continue;

          // CRITICAL: Only flag if the element is visibly rendered with real size
          // This filters out hidden background extension nodes
          ...
          // Don't report the same selector twice
          if (reportedAiSelectorsRef.current.has(sel)) continue;
          reportedAiSelectorsRef.current.add(sel);
          lastAiWarnTimeRef.current = now;

          toast.error('AI assistance is not allowed during the assessment.', { duration: 5000 });
          recordViolation(`AI Assistant Overlay Active (${sel} — ${Math.round(rect.width)}x${Math.round(rect.height)}px)`, 'aiWarnings');
          break;
        } catch (e) {}
      }
    }, 8000); // Check every 8 seconds to reduce CPU usage
  };
  ...
  const startFaceTracking = () => {
    faceCheckIntervalRef.current = setInterval(async () => {
      ...
      if (detections.length !== 1) {
        ...
        if (warningCount >= 3) {
          recordViolation(detections.length === 0 ? 'Face not detected (AI Warning)' : 'Multiple faces detected (AI Warning)', 'aiWarnings');
        } else {
          toast.error(`Warning ${warningCount}/3: ${detections.length === 0 ? 'Face not detected' : 'Multiple faces detected'}. Please look at camera.`);
        }
      }
    }, 3500);
  };
  ...
  // Multi-session / Multi-tab detection
  useEffect(() => {
    if (stage === 'running') {
      try {
        const channel = new BroadcastChannel(`assessment_session_${assessment._id}_${user.email}`);
        channel.postMessage('NEW_SESSION_STARTED');
        channel.onmessage = (msg) => {
          if (msg.data === 'NEW_SESSION_STARTED') {
            recordViolation('Multiple Assessment Sessions Detected', 'tabSwitches');
          }
        };
        return () => channel.close();
      } catch (e) {}
    }
  }, [stage, assessment._id, user.email]);
  ...
  const submitAssessment = async (terminated: boolean) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setStage('result');
        if (terminated) {
          toast.error('Assessment terminated due to a security violation policy.');
        } else {
      ...
      <div className="h-full flex items-center justify-center p-6" style={{ background: 'var(--brand-dark)', minHeight: '100vh' }}>
        <div className="glass-panel p-8 rounded-2xl max-w-3xl w-full">
          ...
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl font-extrabold text-white mb-1">Secure Assessment Mode</h2>
            <p className="text-gray-400">Strict proctoring &amp; browser security enabled. Full Screen required.</p>
          </div>
          ...
          <div className="p-4 mb-6 rounded-xl flex items-start gap-3 text-left bg-red-950/30 border border-red-500/30">
            <AlertTriangle className="w-5 h-5 text-red-400 flex-shrink-0 mt-0.5" />
            <div className="text-xs text-red-200 leading-relaxed">
              <strong className="block mb-1 font-bold text-sm text-red-100">🔒 Strict Security Restrictions Active</strong>
              Right click, Copy/Paste, DevTools (F12/Ctrl+Shift+I), Tab Switching, Fullscreen exit, and AI Extension overlays are strictly disabled &amp; monitored. Violations will result in automatic test termination.
            </div>
          </div>

          <div className="flex gap-4 justify-center">
            <button onClick={onExit} className="px-6 py-3 rounded-xl font-bold text-gray-400 hover:text-white transition-colors cursor-pointer">
              Cancel
            </button>
            <button
              onClick={handleStart}
              ...
              className="px-8 py-3 rounded-xl font-bold text-white flex items-center gap-2 transition-all disabled:opacity-50 cursor-pointer"
              style={{ background: 'var(--color-accent)' }}
            >
              <Play className="w-5 h-5" /> Start Secure Assessment
            </button>
          </div>
        </div>
      </div>
  ...
  if (stage === 'running') {
    ...
    return (
      <div 
        ...
        style={{ background: '#0f1115', userSelect: 'none', WebkitUserSelect: 'none', MozUserSelect: 'none' }}
      >
        {/* Fullscreen Return Modal */}
        {showFullscreenModal && (
          <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center animate-fadeIn">
            <div className="bg-[#1a1a24] border border-amber-500/40 rounded-2xl p-8 max-w-md w-full shadow-2xl space-y-6">
              ...
              <div>
                <h3 className="text-2xl font-bold text-white mb-2">Full Screen Mode Required</h3>
                <p className="text-sm text-gray-300">
                  Assessment security policy requires full screen mode. Exiting full screen has been recorded as a security violation.
                </p>
              </div>
              <button
                onClick={() => {
                ...
                className="w-full py-3.5 bg-amber-500 hover:bg-amber-400 text-amber-950 font-extrabold rounded-xl transition-all cursor-pointer shadow-lg"
              >
        ...
        <div className="h-16 flex items-center justify-between px-6 border-b border-white/10" style={{ background: 'rgba(0,0,0,0.6)' }}>
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] bg-red-500/20 border border-red-500/40 text-red-300 font-bold px-2 py-0.5 rounded-full">SECURE MODE</span>
          </div>
        ...
        {/* Question Content Container with Protection */}
        <div 
          className="flex-1 overflow-y-auto p-6 md:p-12 max-w-4xl mx-auto w-full select-none"
        ...
        >
          ...
          <h2 className="text-2xl font-bold text-white leading-relaxed mb-2 whitespace-pre-wrap select-none" style={{ userSelect: 'none' }}>
            {currentQuestion?.questionText}
          </h2>
          <div className="flex gap-4 text-sm text-gray-500 mb-8 select-none">
              ...
              {currentQuestion.options.map((opt: string, i: number) => {
                ...
                return (
                  <button key={i} onClick={() => handleAnswerChange(opt)}
                    className="w-full text-left p-4 rounded-xl border transition-all flex items-center gap-4 cursor-pointer select-none"
                    style={{
          ...
          <div className="mt-12 flex justify-end">
            <button onClick={handleNext}
              className="px-8 py-3 rounded-xl font-bold text-white flex items-center gap-2 transition-all hover:opacity-90 cursor-pointer"
              style={{ background: 'var(--color-accent)' }}>
    ...
    return (
      <div className="h-full overflow-y-auto p-6 max-w-4xl mx-auto w-full" style={{ animation: 'fadeIn .4s ease' }}>
            ...
            {result.passed && assessment.certificateEligible ? (
              <>
                ...
                <button onClick={generateCertificate}
                  className="px-6 py-3 rounded-xl font-bold text-black flex items-center gap-2 transition-all hover:opacity-90 cursor-pointer"
              ...
              <>
                ...
                <h3 className="text-xl font-bold text-gray-400 mb-2">No Certificate Issued</h3>
                <p className="text-sm text-gray-500">
                  {!result.passed
                    ? 'Score above passing percentage to earn a certificate.'
                    : 'This assessment is not eligible for certification.'}
                </p>
              </>
            )}
          </div>
        ...
        <div className="flex justify-center">
          <button onClick={onExit} className="px-8 py-3 rounded-xl font-bold text-white bg-gray-700 hover:bg-gray-600 transition-colors cursor-pointer">
            Return to Dashboard
          </button>
        </div>
      </div>
