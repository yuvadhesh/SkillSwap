...
export default function AdminTab({ currentUser }: { currentUser: any }) {
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const lastName = newUserForm.name.split(' ').slice(1).join(' ') || firstName;
  ...
  const triggerSeed = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/admin/seed`, { method: 'POST' });
      const data = await res.json();
      if (data.success) {
  ...
  const triggerWipe = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/admin/wipe`, { method: 'POST' });
      const data = await res.json();
      if (data.success) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-white p-4 rounded-xl border border-border">
        ...
        <div className="flex gap-2">
          <button 
            ...
            disabled={loading}
            className="flex items-center gap-1.5 px-3 py-2.5 rounded-md text-xs font-semibold border border-border bg-secondary hover:bg-muted text-foreground cursor-pointer transition-colors min-h-[44px]"
          >
          ...
          <button 
            ...
            className="flex items-center gap-1.5 px-3 py-2.5 rounded-md text-xs font-semibold bg-[var(--brand)] hover:bg-[var(--brand-mid)] text-white cursor-pointer transition-colors shadow-sm min-h-[44px]"
          >
                        ...
                        <td className="p-4 text-right">
                          <div className="flex justify-end gap-1.5">
                            <button
                              ...
                              className="p-2 rounded hover:bg-secondary border border-transparent hover:border-border text-foreground transition-all cursor-pointer min-h-[44px] min-w-[44px] flex items-center justify-center"
                              title="Edit Profile"
                            >
                            ...
                            <button
                              ...
                              className={`p-2 rounded hover:bg-red-50 border border-transparent hover:border-red-100 text-red-600 transition-all min-h-[44px] min-w-[44px] flex items-center justify-center ${isSystemAdmin ? 'opacity-35 cursor-not-allowed' : 'cursor-pointer'}`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fadeIn">
          ...
          <div className="bg-white border border-border rounded-xl p-5 shadow-sm space-y-4 flex flex-col justify-between">
            ...
            <div>
              <button 
                onClick={triggerSeed}
                className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-md text-xs font-semibold cursor-pointer transition-colors shadow-sm min-h-[44px]"
              >
                Seed Mock Users
              </button>
            </div>
          ...
          <div className="bg-white border border-destructive/30 rounded-xl p-5 shadow-sm space-y-4 flex flex-col justify-between">
            ...
            <div>
              <button 
                onClick={triggerWipe}
                className="w-full py-3 border border-destructive/40 bg-destructive/5 text-destructive hover:bg-destructive/10 rounded-md text-xs font-semibold cursor-pointer transition-colors min-h-[44px]"
              >
